...
# Redirección para Streamlit Cloud. ¡MUY IMPORTANTE!
# Esta URL DEBE COINCIDIR EXACTAMENTE con una de las "Authorized redirect URIs"
# configuradas en tu proyecto de Google Cloud Console para tu OAuth 2.0 Client ID.
# Si estás ejecutando la app localmente, usualmente es "http://localhost:8501".
# Si la has desplegado en Streamlit Cloud, será la URL pública de tu app (ej. "https://your-app-name.streamlit.app/").
GOOGLE_REDIRECT_URI = "https://groceries-00.streamlit.app/" # <--- ¡VERIFICA ESTA URL!

# Mensaje de advertencia si la app se ejecuta localmente y la URI no es localhost
if "localhost" in st.experimental_get_query_params() or "127.0.0.1" in st.experimental_get_query_params():
    if GOOGLE_REDIRECT_URI != "http://localhost:8501":
        st.info(f"💡 Estás ejecutando la app localmente. Asegúrate de que `GOOGLE_REDIRECT_URI` en el código sea `http://localhost:8501` y que esta URL esté configurada en Google Cloud Console. Actualmente es: `{GOOGLE_REDIRECT_URI}`")
else:
    st.info(f"💡 Asegúrate de que `GOOGLE_REDIRECT_URI` (`{GOOGLE_REDIRECT_URI}`) coincida con la URL de tu aplicación desplegada en Streamlit Cloud y que esté configurada en Google Cloud Console.")
...
if __name__ == "__main__":
    ...
    # Then handle the OAuth callback
