...
def get_redirect_uri():
    ...
    # Para producción (cambia 'tu-app-name' por el nombre real de tu app)
    else:
        return "https://groceries-00.streamlit.app/"
...
def login_screen():
    ...
    st.markdown(f'<a href="{auth_url}" target="_self"><button style="background: #4285f4; color: white; border: none; padding: 12px 24px; border-radius: 8px; font-size: 16px; cursor: pointer;">🔐 Iniciar Sesión con Google</button></a>', unsafe_allow_html=True)
...
def main():
    ...
    if not st.session_state.user_authenticated:
        ...
        if saved_user_data and saved_user_data.get('authenticated'):
            st.session_state.user_authenticated = True
            st.session_state.user_info = saved_user_data.get('user_info')
            st.session_state.access_token = saved_user_data.get('access_token')
