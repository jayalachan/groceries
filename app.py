...
import json
import os
import secrets # Aunque no se usa directamente para la generación de secretos, es útil para recordar su propósito
...
client_config = {
    "web": {
        ...
        "redirect_uris": [
            ...
            # st.experimental_get_query_params() es útil para obtener la URL dinámica en Streamlit Cloud
            f"https://{st.experimental_get_query_params().get('__streamlit_host__', ['localhost'])[0]}.streamlit.app"
        ]
    }
}
...
# La REDIRECT_URI para el flujo OAuth. Usaremos la primera de las definidas en client_config para simplificar,
# pero en un despliegue real en Streamlit Cloud, Google redirigirá a la URI de Streamlit Cloud.
# Lo importante es que esta URI esté en la lista de 'redirect_uris' en Google Cloud Console.
# Para Streamlit Cloud, la URI de redirección efectiva será la de tu app desplegada.
REDIRECT_URI_FOR_FLOW = client_config["web"]["redirect_uris"][1] # Usamos la de Streamlit Cloud
