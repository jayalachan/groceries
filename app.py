...
# Es crucial que esta estructura coincida con lo que Google espera.
client_config = {
    "web": {
        "client_id": st.secrets["GOOGLE_CLIENT_ID"],
        # project_id es opcional para el flujo, pero incluirlo es buena práctica si lo tienes.
        ...
        "redirect_uris": [
            # URI para desarrollo local (Streamlit usa por defecto localhost:8501)
            "http://localhost:8501",
            # URI para despliegue en Streamlit Cloud
            # Utiliza st.query_params para la URL dinámica en Streamlit Cloud.
            # Asegúrate de que esta URL también esté configurada en Google Cloud Console para tu OAuth Client ID.
            f"https://{st.query_params.get('__streamlit_host__', 'localhost').split(':')[0]}.streamlit.app"
        ]
    }
}
...
# La REDIRECT_URI para el flujo OAuth. Usaremos la de Streamlit Cloud para el flujo.
# Lo importante es que esta URI esté en la lista de 'redirect_uris' en Google Cloud Console.
REDIRECT_URI_FOR_FLOW = client_config["web"]["redirect_uris"][1] # Usamos la de Streamlit Cloud (índice 1)
...
def google_oauth_login():
    ...
    """
    flow = Flow.from_client_config( # Usar from_client_config
        client_config,               # Pasar el diccionario client_config
        scopes=SCOPES,
        redirect_uri=REDIRECT_URI_FOR_FLOW # Usar la URI para el flujo
    )
...
def handle_oauth_callback():
    ...
    """
    # Usar st.query_params directamente
    ...
    if 'code' in query_params and 'state' in query_params and query_params['state'] == st.session_state.get('oauth_state'):
        ...
        try:
            ...
            # Verificar el token con el client_id obtenido de secrets
            ...
            # Limpiar los parámetros de la URL. Si esto causa un rerun no deseado,
            # considera limpiar los parámetros de la URL después de un redirigir si es posible,
            # o manejar el estado de la aplicación para que no se vea afectado por ellos.
            ...
            st.query_params.clear() # Limpiar en caso de error también
            return False
