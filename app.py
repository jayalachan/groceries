...
REDIRECT_URI_FOR_FLOW = "https://groceries-00.streamlit.app/" # Ensure this is correct in Google Cloud Console
...
client_config = {
    "web": {
        ...
        "redirect_uris": [
            ...
            REDIRECT_URI_FOR_FLOW    # Streamlit Cloud deployment
        ]
    }
}
...
# Your existing session_state initializations...
...
# Crucial for state management in OAuth:
...
# --- Funciones de Gestión de Datos (Your existing functions) ---
...
def google_oauth_login():
    """Inicia el flujo de autenticación de Google OAuth usando st.link_button."""
    flow = Flow.from_client_config(
        client_config,
        scopes=SCOPES,
        redirect_uri=REDIRECT_URI_FOR_FLOW
    )
    # Generate a new state each time a user clicks the login button
    authorization_url, state = flow.authorization_url(
        access_type='offline',
        include_granted_scopes='true',
        prompt='consent' # Ensure prompt is set to 'consent' for refresh tokens if needed
    )
    st.session_state['oauth_state'] = state # Store the state in session_state

    # Use st.link_button for the redirect
    st.link_button(
        "▶️ Iniciar sesión con Google",
        authorization_url,
...
def handle_oauth_callback():
    ...
    # Check for 'state' and 'code' in query parameters
    if 'code' in query_params and 'state' in query_params:
        ...
        if state_param == st.session_state.get('oauth_state'):
            ...
            try:
                ...
                # Clear query parameters and oauth_state after successful authentication
                st.session_state['oauth_state'] = None # Crucial: Clear the state after use
                ...
                st.session_state['oauth_state'] = None # Clear state on failure too
                st.query_params.clear()
                return False # Indicate failure
        else:
            # This is the primary cause of the error message you're seeing
            st.warning("¡Estado de OAuth inválido! Intenta iniciar sesión de nuevo.")
            st.session_state['oauth_state'] = None # Clear potentially stale state
            st.query_params.clear()
            return False # Indicate failure
        ...
        st.session_state['oauth_state'] = None # Clear state on error
        st.query_params.clear()
        return False # Indicate failure
    return None # No callback parameters found
...
def logout():
    ...
    if 'oauth_state' in st.session_state:
        del st.session_state['oauth_state'] # Ensure oauth_state is cleared on logout
    st.success("Sesión cerrada correctamente.")
    st.rerun()

# --- Rest of your functions (add_product_to_master, etc.) ---
...
# Handle callback at the very beginning of the script execution
auth_result = handle_oauth_callback()

if auth_result is True:
    load_user_data()
    st.rerun() # Rerun to clear query params and show logged-in state
elif auth_result is False:
    # If there was an error or invalid state, stop execution to prevent further issues
    # and allow the user to try logging in again.
    st.stop() # This prevents the rest of the app from rendering in an error state

# This block will only be reached if auth_result is None (no callback params) or after a successful rerun
